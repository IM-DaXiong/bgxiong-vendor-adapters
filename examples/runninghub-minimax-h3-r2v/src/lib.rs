...
use crate::runninghub::{
    outputs_to_json, parse_model_kind, parse_query, parse_submit_task_id, submit_body_mapped,
    MappedStatus,
};
...
fn submit_op(payload_json: &str) -> Result<serde_json::Value, String> {
    ...
    let refs = crate::runninghub::collect_r2v_refs(&payload, kind.max_refs())?;
    let uploaded: Vec<&str> = refs.iter().map(|s| s.handle.as_str()).collect();
    let _url = kind.submit_url();
    let _body = submit_body_mapped(kind, prompt, duration, fps, resolution, aspect, &uploaded)?;
...
mod tests {
    ...
    fn capabilities_returns_rcd() {
        ...
        assert_eq!(v["slots"][2]["implementedModeIds"], serde_json::json!(["multi_image_to_video"]));
        ...
        assert_eq!(v["slots"][0]["duration"]["bindingId"], "134:value");
        assert_eq!(v["slots"][0]["fps"]["bindingId"], "131:fps");
        assert_eq!(v["slots"][1]["duration"]["bindingId"], "132:value");
        assert_eq!(v["slots"][1]["fps"]["bindingId"], "130:fps");
    ...
    fn submit_without_guest_http_fails_explicitly() {
        let err = dispatch_json(
            "submit",
            r#"{"model":"h3.r2v.1slot","prompt":"cat","durationSeconds":5,"fps":25,"resolution":"mp:0.4","aspectRatio":"16:9","startFrame":{"handle":"h1","fileName":"a.png","mime":"image/png"}}"#,
        )
        .unwrap_err();
    ...
    fn http_200_body_failed_is_failed() {
        ...
        assert_eq!(data["terminalFailure"]["vendorCode"], "FAILED");
        assert_eq!(data["terminalFailure"]["message"], "no");
        assert!(data.get("progressText").is_none() || data["progressText"].is_null());
    }
}
