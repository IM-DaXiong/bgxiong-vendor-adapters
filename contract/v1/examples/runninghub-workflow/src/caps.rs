...
pub fn runtime_caps_doc() -> Result<Value, String> {
    ...
    if let Some(b) = bound(FPS_NODE_ID) {
        insert_param(
            ...
            RcdParam::enum_of(
                vec![
                    json!({"id": "8", "label": "8", "value": 8}),
                    json!({"id": "12", "label": "12", "value": 12}),
                    json!({"id": "16", "label": "16", "value": 16}),
    ...
    if let Some(b) = bound(RESOLUTION_NODE_ID) {
        insert_param(
            ...
            RcdParam::enum_of(
                vec![
                    json!({"id": "16:9", "label": "16:9", "value": "1280x720"}),
                    json!({"id": "9:16", "label": "9:16", "value": "720x1280"}),
                    json!({"id": "1:1", "label": "1:1", "value": "1024x1024"}),
                ],
                json!("1280x720"),
                b,
            ),
